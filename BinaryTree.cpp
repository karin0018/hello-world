 ...
 * @Last Modified time: 2019-10-29 21:31:41
...
bNode *buildTreeByvalue(int *value, int len) {
  ...
  for (idx = 1; idx < len; idx++) {  //逐个创建二叉树的新节点
    ...
    bNode *parent;  // p的双亲节点
    for (bNode *p = root; p;
         (value[idx] > p->data.value) ? p = p->rchild : p = p->rchild)
      parent = p;
...
int MAXVALUE ;
int MINVALUE ;//注意不能预先定义具体数值，应该针对每一个新树，选取新树中的值预定义，否则 若预先定义 0 但新树中没有比 0 小的数值，则范围出错

void MaxVal(bNode *root) {
  if (root) {
...
int main() {
  ...
  getchar();
  printf("inTraverse tree \n");
  inTraverseTree(tree);
  getchar();
  printf("postTraverse tree \n");
  postTraverseTree(tree);
  getchar();
  printf("seqTraverse tree \n");
  seqTraverseTree(tree);
  getchar();
  MAXVALUE=tree->data.value;
  MINVALUE=tree->data.value;
